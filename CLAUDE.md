...
- **Bewertungs-/Marktengine:** `src/lib/valuation.ts` + `src/lib/marktdaten.ts`
  ...
  `src/lib/stadt-faktor.ts` (27 Orts-Faktoren) + `src/lib/stadt-niveau.ts` (20 Städte,
  amtlich/recherchiert — beide region-spezifisch, s. Playbook §7);
...
- `docs/naechste-schritte.md` — **belegte Bottleneck-Analyse + priorisierter Plan**
  (Wellen 1–4, jede Zahl gemessen; Stand 18.08.2026). Erledigtes dort abhaken.
