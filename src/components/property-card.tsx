...
export function PropertyCard({ estate }: { estate: Estate }) {
  return (
    <Link
      href={`/immobilien/${estate.slug}`}
      className="group block overflow-hidden rounded-xl border border-border bg-surface transition-[border-color,transform] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:border-accent/60"
    >
      ...
      <div className="space-y-3 p-5">
        ...
        <div className="flex items-center justify-between pt-2">
          <span className="text-lg font-semibold text-accent">{estate.price}</span>
