...
| B | **€-Pipeline im Cockpit** ⭐ | Jeder Lead in `/intern`: Wert × Provisionssatz = Potenzial; Dashboard-Summe „Pipeline diesen Monat: X €" | Der Demo-Moment fürs Pitch-Meeting; macht Website-Wert sichtbar | klein — Cockpit mit KPIs/Tabs/CSV existiert; **fehlt nur €-Potenzial je Lead + Summen-KPI** |
| C | **PDF-Report-Funnel** | Ergebnis bleibt frei (No-Gate-USP!), Report per E-Mail + CTA „kostenlose Vor-Ort-Präzisierung"; anonyme Bewertungen als Nachfrage-Signal ins Cockpit | Kontakt entsteht freiwillig | **teilweise live**: `/api/report` (HTML-Report + CC an RIEGEL, `valuation_requests`-Log). Fehlt: echtes PDF (`report-pdf.ts` anbinden) + Vor-Ort-CTA |
